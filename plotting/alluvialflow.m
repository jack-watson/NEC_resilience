function h = alluvialflow(data, left_labels, right_labels, left_ranks, right_ranks, opts)
% Copyright 2018 The MathWorks, Inc. 
% Published on FileExchange by Alexander Carmeli (MathWorks Staff)
% 12 April 2018, Version 1.0.0.0

% FileExchange "Cite As" copypaste:
% "Alexander Carmeli (2025). Alluvial flow diagram 
% (https://www.mathworks.com/matlabcentral/fileexchange/66746-alluvial-flow-diagram), 
% MATLAB Central File Exchange. Retrieved November 10, 2025."

% MODIFIED by Jack R. Watson
% Changed how colors are specified to allow for fine-tuned behavior
...
if nargin < 6 || ~isstruct(opts)
    opts = struct;
    opts.textLabels = true;
end

if ~isfield(opts, "rankFontSize")
    opts.rankFontSize = 10;
end
...
hold on; patch([0 0 1 1], [0 y_height y_height 0], 'w');

left_category_sizes = sum(data, 2)'; % Plot left categories - one per row
...
% Color selection
if ~isfield(opts, "patchColors")
    patch_colors = [ .5 .5 .5;
        1  0  0;
        0.3686    0.5137    0.9686;
        0  1  0;
        0  0  1;
        1.0000    0.5922         0;
        0.4000    0.9765    0.4157;
        0 .5 .5;
        .5  0 .5;
        0.9490    0.4118    0.3569;
        0.4000    0.9765    0.8902;
        0.9765    0.7059    0.4000];
else
...
if opts.textLabels % Place text labels if specified (defaults to true)
    text(zeros(1, size(data, 1)) - 0.03, ... % Place left labels
        left_category_points + left_category_sizes./2, ...
        left_labels, 'FontSize', 10, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'Rotation', 30,...
        Interpreter="latex");

    text(ones(1, size(data, 2)) + 0.03, ...  % Place right labels
        right_category_points + right_category_sizes./2 - 0.15, ...
        right_labels, 'FontSize', 10, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'Rotation',30,...
        Interpreter="latex");
end

text(zeros(1, size(data, 1)) + 0.043, ... % place rankings
    left_category_points + left_category_sizes./2 + 0.15, ...
    "$\#" + string(left_ranks) + "$", 'FontSize', opts.rankFontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'Rotation', 0,...
    Interpreter="latex");

text(ones(1, size(data, 2)) - 0.043, ...
    right_category_points + right_category_sizes./2 - 0.15, ...
    "$\#" + string(right_ranks) + "$", 'FontSize', opts.rankFontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'Rotation',0,...
    Interpreter="latex");

if nargin >= 6 && isstruct(opts) && isfield(opts, "pltTitle") && ~isempty(opts.pltTitle)
    title(opts.pltTitle);
end
